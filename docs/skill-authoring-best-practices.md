...
- Vendored skills keep their upstream wording. Skills imported from a vendor or another author (for example `hatch-pet`, `chronicle`, `terraform-skill`, anything carrying its own `license` or `author` frontmatter) are not ours to trim or restyle; leave their descriptions and bodies intact and route around them instead.
