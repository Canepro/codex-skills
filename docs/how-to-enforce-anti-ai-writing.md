...
Already done if you cloned this repo and ran `scripts/bootstrap.sh`. The skill is installed at `~/.claude/skills/anti-ai-writing/SKILL.md` (directory form, picked up by the Skill tool) and `~/.claude/skills/anti-ai-writing.md` (legacy flat form). The paste block lives in `~/.claude/CLAUDE.md` under "Writing style (always)" and loads into every Claude Code session.
