...
Each log entry already has a metadata timestamp, so the inline timestamp is redundant (~30-34 bytes each, ~6% of a typical log line).
...
Starting at 753 bytes (minified), after removing nulls, placeholders, unused fields, and normalizing keys: **464 bytes, a 38% reduction**.
